Para adicionar persistência em banco de dados à nossa aplicação CRUD com NestJS, vamos usar o TypeORM com um banco de dados **MySQL**. 
...
export class ContactsService {
  ...
  async findOne(id: number): Promise<Contact | null> {
    return this.contactsRepository.findOneBy({ id }); // id já convertido para number no controller
  }
...
export class ContactsController {
  ...
  findOne(@Param('id') id: string): Promise<Contact | null> {
    return this.contactsService.findOne(Number(id)); // parâmetros de rota vêm como string
  }
