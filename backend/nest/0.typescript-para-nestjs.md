...
```mermaid
flowchart TB
    Dec["@Controller / @Get / @Injectable"] --> Meta[Metadata TypeScript]
    Meta --> Nest[Nest lê e registra]
    Nest --> Rotas[Rotas HTTP]
    Nest --> DI[Injeção de dependência]
```
...
flowchart LR
    D["@Get(':id')"] --> Nest[Nest]
    Nest --> Anuncio["anuncia: GET /products/:id → findOne"]
```
