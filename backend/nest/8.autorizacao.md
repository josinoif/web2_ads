...
TOKEN=$(curl -X POST http://localhost:3000/auth/register -d '{"username": "client", "password": "client", "email": "client@gmail.com", "role": "CLIENT"}' -H 'Content-Type: application/json' | jq -r .access_token)


# Obter o token de acesso
TOKEN=$(curl -X POST http://localhost:3000/auth/login -d '{"username":"client", "password":"client"}' -H 'Content-Type: application/json' | jq -r .access_token)
