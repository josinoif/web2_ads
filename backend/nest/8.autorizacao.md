...
export class AuthService {
  ...
  async validateUser(username: string, pass: string): Promise<any> {
    ...
    console.log('Validating user:', username, 'Found user:', user);
...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: any) {
    return { userId: payload.sub, username: payload.username, role: payload.role }; // altere essa linha
  }
}
```
...
export class AuthController {
  ...
  async register(@Body() body: { username: string; password: string, email:string,  role: string }) {
    return this.authService.register(body.username, body.password, body.email, body.role);
  }
}
```
...
```
## 9. Testar a API
Você pode usar curl para testar os endpoints protegidos por papéis. O token JWT é retornado pelo endpoint de **login** (não pelo de registro).

```bash
# 1. Registrar um novo usuário (resposta é o objeto do usuário, não o token)
curl -X POST http://localhost:3000/auth/register -d '{"username": "client", "password": "client", "email": "client@gmail.com", "role": "CLIENT"}' -H 'Content-Type: application/json'

# 2. Fazer login e guardar o token na variável TOKEN (requer jq: sudo dnf install jq)
TOKEN=$(curl -s -X POST http://localhost:3000/auth/login -d '{"username":"client", "password":"client"}' -H 'Content-Type: application/json' | jq -r .access_token)
