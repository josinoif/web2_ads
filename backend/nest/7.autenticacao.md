...
Esse tutorial evolui o código desenvolvido no [Capítulo 5](https://github.com/josinoif/web2_ads/blob/main/backend/nest/5.crud_nest_bd.md) desse material. Caso não tenha esse código, refaça o capítulo 5 e depois continue nesse módulo.
...
```bash
curl -X POST http://localhost:3000/auth/register -H "Content-Type: application/json" -d '{"username": "aluno", "password": "aluno", "email": "email@gmail.com"}'
```

o retorno da aplicação deverá ser algo como o JSON a seguir (em produção, evite devolver o campo `password` na resposta; você pode usar um DTO ou `class-transformer` com `@Exclude()` para omiti-lo):
