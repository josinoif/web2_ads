...
- `entities`: Padrão de glob que indica onde estão as entidades (arquivos `*.entity.ts` ou `*.entity.js`). Alternativa: usar `autoLoadEntities: true` e registrar entidades apenas com `TypeOrmModule.forFeature([...])` em cada módulo.
- `synchronize`: Sincroniza automaticamente o esquema do banco com as entidades (**use apenas em desenvolvimento**; em produção prefira migrations).
