...
Neste tutorial, usaremos o **TypeORM** e o banco de dados **PostgreSQL**.
...
export class User {
  ...
  @JoinColumn() // Define o lado da FK
  profile: Profile;
}
```
...
1. **Como isso se traduz no banco de dados**
...
| ---- | ------ | ---------- | ------ |
| 1    | Post 1 | Conteúdo 1 | 1      |
| 2    | Post 2 | Conteúdo 2 | 1      |
| 3    | Post 3 | Conteúdo 3 | 2      |

#### Explicação

- A coluna `userId` na tabela `posts` é uma chave estrangeira que referencia a tabela `users`.
- O decorador `@OneToMany` na entidade `User` define o lado "um", enquanto o decorador `@ManyToOne` na entidade `Post` define o lado "muitos".
...
**Tabela intermediária `users_groups`**:
