...
export class FilesService {
  getFilePath(filename: string): string {
    return join(__dirname, '../../upload', filename);
  }
}
```
...
2. **Método `getFilePath`**:
   - Usa `join` do módulo **path** do Node.js para construir o caminho absoluto do arquivo na pasta `upload`.
...
- Arquivos enviados serão armazenados na pasta `upload`.
