...
class Handler(JsonHandler):
    def do_GET(self) -> None:
        ...
        if path == "/health":
            self._json(200, {"ok": True, "service": "store-sync", "itens": len(STORE)})
            return
    ...
    def do_POST(self) -> None:
        ...
        submission_id = data.get("submission_id") or f"sync-{uuid.uuid4().hex[:8]}"
        registro = {
...
if __name__ == "__main__":
    print(f"[store-sync] :{PORT}", flush=True)
